  ...
  return (
    <div className="bg-white rounded-lg py-4 mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {communities
          .slice(0, getAll ? communities.length : 4)
          .map((community) => (
            <motion.div
      ...
      <button
        className="mt-4 w-full bg-indigo-500 text-white py-2 rounded-lg hover:bg-indigo-600 transition duration-300 ease-in-out"
        onClick={showAll}
      >
        {getAll ? " See Less Communities " : "Explore All Communities"}
      </button>
    </div>
  );
};
